...
- Admin access is whitelisted: edit `ADMIN_EMAILS` at the top of `admin/js/app.js` (keep in sync with the email list inside `firestore.rules`). Admin auth uses `browserSessionPersistence`; any non-whitelisted account (learner Google logins) is auto-signed-out of the panel.
